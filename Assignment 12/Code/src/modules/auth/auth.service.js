...
const verifyEmailOtp = async ({email, subject = emailEnum.ConfirmEmail, title = "Verify Account"} = {}) => {
  ...
  if (remainingBlockTime > 0) {
    throw ConflictException({message:`You have reached max trial count please try again after ${remainingBlockTime} seconds`})
  }

  const oldCodeTTl = await ttl(otpKey({email,type:subject}))
  if (oldCodeTTl > 0 ) {
    throw ConflictException({message:`Sorry we cannot send new otp until the first one expires, try again after ${oldCodeTTl}`})
  }
  ...
  if (checkMaxOtpRequest >= 3) {
    ...
    throw ConflictException({message:`You have reached max trial count please try again after 300 seconds`})
  }
